...
import { LogOut, Users, Activity, CreditCard, Shield, Pencil, Camera, Upload, Check, X } from 'lucide-react';
            ...
            <button type="button" onClick={() => fileRef.current?.click()}
              ...
              {preview
                ...
                : <div className="w-24 h-24 flex flex-col items-center justify-center gap-1">
                    <Camera className="w-6 h-6 text-primary/50" /></div>}
            </button>
          ...
          {[
            ...
            <div key={f.label}>
              ...
              <input type={f.type} value={f.value} onChange={e => f.setValue(e.target.value)}
                className="w-full h-11 px-4 rounded-xl border border-surface-border bg-white text-sm text-ink focus:outline-none focus:border-primary" />
            </div>
          ))}
...
export default function AdminProfile() {
  ...
  const initials  = currentUser?.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || 'AD';
  const walkers   = data.users.filter(u => u.role === 'walker');
  const owners    = data.users.filter(u => u.role === 'owner');
  const completed = data.walks.filter(w => w.status === 'completed');
  const payments  = data.payments;

  const handleLogout = () => { logout(); navigate('/login'); };

  const stats = [
    { icon: Users,    label: 'Walkers',  value: walkers.length },
    { icon: Users,    label: 'Owners',   value: owners.length  },
    { icon: Activity, label: 'Walks',    value: completed.length },
  ...
  return (
    <div className="max-w-xl mx-auto pb-28">
      ...
      <div className="relative overflow-hidden px-5 pt-10 pb-0"
        style={{ background: 'linear-gradient(135deg, #14532D 0%, #1B4332 50%, #166534 100%)' }}>
      ...
      <div className="px-4 pt-5 space-y-4">
        ...
        <div className="bg-white border border-surface-border rounded-2xl overflow-hidden divide-y divide-surface-border">
          {[
            { label: 'Manage Walkers', path: '/admin/walkers',  emoji: '🦮' },
            { label: 'Manage Owners',  path: '/admin/owners',   emoji: '👤' },
            { label: 'All Walks',      path: '/admin/walks',    emoji: '🗓️' },
            { label: 'Payments',       path: '/admin/payments', emoji: '💳' },
            { label: 'Analytics',      path: '/admin/analytics',emoji: '📊' },
          ].map(item => (
            <button key={item.path} type="button" onClick={() => navigate(item.path)}
              className="w-full flex items-center gap-3 px-4 py-4 hover:bg-surface-secondary transition-colors text-left">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center text-xl shrink-0" style={{ background: '#EBF5EF' }}>
                {item.emoji}
              </div>
